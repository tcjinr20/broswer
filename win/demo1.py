...
def OnPaint_1(hwnd, msg, wp, lp):
    ...
    win32gui.SetWorldTransform(dc,
                               {'M11': math.cos(r_angle), 'M12': math.sin(r_angle), 'M21': math.sin(r_angle) * -1,
                                'M22': math.cos(r_angle), 'Dx': 250, 'Dy': 250})
...
def OnPaint_2(hwnd, msg, wp, lp):
    ...
    for x in range(25):
        vertices = (
            {'x': int(random.random() * r), 'y': int(random.random() * b), 'Red': int(random.random() * 0xff00),
             'Green': 0, 'Blue': 0, 'Alpha': 0},
            {'x': int(random.random() * r), 'y': int(random.random() * b), 'Red': 0,
             'Green': int(random.random() * 0xff00), 'Blue': 0, 'Alpha': 0},
            {'x': int(random.random() * r), 'y': int(random.random() * b), 'Red': 0, 'Green': 0,
             'Blue': int(random.random() * 0xff00), 'Alpha': 0},
        )
        mesh = ((0, 1, 2),)
