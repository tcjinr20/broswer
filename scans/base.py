...
'''
from scans.pycui  import *
from scans.genips import genips
...
class Scans(threading.Thread):
	...
	def checkProxy(self, h, p):
		_p = "http://%s:%s"%(h, p)
		_h=request.ProxyHandler({'http': _p})
		_o = request.build_opener(_h, request.HTTPHandler)
		request.install_opener(_o)
		# _h = urllib2.ProxyHandler({'http': _p})
		# _o = urllib2.build_opener(_h, urllib2.HTTPHandler)

		try:
			_r = request.urlopen('http://www.baidu.com/img/baidu_jgylogo3.gif')
